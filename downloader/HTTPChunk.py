...
class ChunkInfo():
    ...
    def save(self):
        fs_name = fs_encode(chunk_folder + split("%s.chunks" % self.name)[-1])
    ...
    @staticmethod
    def load(name):
        fs_name = fs_encode(chunk_folder + split("%s.chunks" % name)[-1])
    ...
    def remove(self):
        fs_name = fs_encode(chunk_folder + "%s.chunks" % self.name)
...
class HTTPChunk(HTTPRequest):
    def __init__(self, id, parent, range=None, resume=False, headers=None):
        self.id = id
    ...
    def getHandle(self):
        ...
        # print(self.resume, self.range, self.arrived)
        # request all bytes, since some servers in russia seems to have a defect arihmetic unit

        fs_name = fs_encode(chunk_folder + os.path.split(self.p.info.getChunkName(self.id))[-1])
        if self.resume:
