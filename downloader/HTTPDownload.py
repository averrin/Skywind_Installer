...
from utils import save_join, fs_encode

import os

chunk_folder = 'config/temp/'
...
class HTTPDownload():
    ...
    def _copyChunks(self):
        init = fs_encode(chunk_folder + os.path.split(self.info.getChunkName(0))[-1]) #initial chunk name
        ...
        if self.info.getCount() > 1:
            ...
            for i in range(1, self.info.getCount()):
                ...
                fname = fs_encode(chunk_folder + os.path.split("%s.chunk%d" % (self.filename, i))[-1])
                fi = open(fname, "rb")
                buf = 32 * 1024
    ...
    def download(self, chunks=1, resume=False):
        ...
        try:
            ...
            if code == 33:
                ...
                print("Errno 33 -> Restart without resume")
    ...
    def _download(self, chunks, resume):
        ...
        while 1:
            ...
            if not chunksCreated and self.chunkSupport and self.size: #will be set later by first chunk
                ...
                #there
            # print('before', chunksCreated, self.chunkSupport, self.size)
            if self.suspended:
                self.callback(self.speed, self.arrived, self.percent, self.size)
                raise Abort()
            ...
            # print(self.filename, self.speed, self.arrived, self.percent, self.size)
        ...
        if self.bf_callback is not None:
            self.bf_callback()
...
if __name__ == "__main__":
    ...
    dwnld = HTTPDownload(url, "test_100mb.jpg", bucket=bucket)
